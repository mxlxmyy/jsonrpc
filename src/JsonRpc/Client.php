...
class Client
{
    ...
    public function addHttpParams($key, $value)
    {
        $this->httpParams[$key] = $value;
    }

    public function clearHttpParams()
    {
    ...
    private function send()
    {
        ...
        try {
            if ($res = $this->transport->send('POST', $this->url, $data, $this->headers, $this->httpParams)) {
    ...
    private function setError($error, $desc = "")
    {
        if (is_string($error)) {
            $code    = 0;
            ...
            $desc    = $desc ?: isset($error->desc) ? $error->desc : '';
        }
