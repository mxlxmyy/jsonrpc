...
class Server
{
     ...
     * 返回特定错误
     */
    public function returnError($code = -32000)
    {
        $this->init();
        try {
            $input  = $this->transport->receive();
            $struct = Rpc::decode($input, $batch);
            $data   = new Response();
            if ($struct->id) {
                $data->createStdError($code, $struct->id);
            } else {
                $data->createStdError($code);
            }
            $this->transport->reply($data->toJson());
        } catch (\Exception $e) {
            $this->logException($e);
            exit;
        }
    ...
    private function getCallback($method)
    {
        ...
        if (method_exists($this->handler, $method) && is_callable($callback)) {
            return $callback;
        }
    }
    ...
    {
        $params = \zmtool\Arr::objectToArray($params);
    }
